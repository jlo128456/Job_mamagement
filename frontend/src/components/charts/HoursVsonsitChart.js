...
import {
  ...
  CartesianGrid, Tooltip, Legend, LabelList, Label
} from "recharts";
...
const aggregate = ({ jobs=[], users=[], metrics=[] }) => {
  ...
  const rows = new Map(); const now = new Date();
  const bump = (uid, lh, oh) => {
    const u = uMap.get(uid); if (!uid || (u?.role && !allowRole(u.role))) return;
    const r = rows.get(uid) || { name: niceName(u), labourHours: 0, onsiteHours: 0 };
    r.labourHours += num(lh); r.onsiteHours += num(oh); rows.set(uid, r);
  };
  if (metrics?.length) {
    metrics.forEach(m => { if (allowRole(m.role)) bump(m.user_id, m.labour_hours, m.onsite_hours); });
  } else {
...
export default function HoursVsOnsiteChart({ jobs, users, metrics=[] }) {
  ...
  const max = Math.max(1, Math.ceil(Math.max(0, ...data.flatMap(d=>[d.labourHours,d.onsiteHours]))));
  return (
    <div style={{ marginTop:16, padding:12, borderRadius:12, background:"#fff", boxShadow:"0 6px 18px rgba(0,0,0,.08)" }}>
      <h3 style={{ margin:"6px 0 12px" }}>Contractor / Technician: Hours vs Onsite</h3>
      <ResponsiveContainer width="100%" height={340}>
        <BarChart data={data} barGap={6} barCategoryGap={24} margin={{ top:8, right:16, left:56, bottom:18 }}>
          <CartesianGrid strokeDasharray="4 4" opacity={0.4} />
          <XAxis dataKey="name" angle={-12} textAnchor="end" height={42} />
          <YAxis domain={[0,max]} tickFormatter={v=>v.toFixed(0)}>
            <Label
              content={({ viewBox }) => {
                const { x=0, y=0, height=0 } = viewBox || {};
                const cx = x + 12, cy = y + height / 2;
                return (
