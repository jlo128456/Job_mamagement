...
function CreateJobModal({ isOpen, onClose }) {
  ...
  const [users, setUsers] = useState([]), [machines, setMachines] = useState([]);
  ...
  useEffect(() => {
    if (!isOpen) return;
    Promise.all([
      fetch(`${API_BASE_URL}/users/staff`).then(r => r.ok && r.json()),
      fetch(`${API_BASE_URL}/machines`).then(r => r.ok && r.json())
    ]).then(([u, m]) => { u && setUsers(u); m && setMachines(m); });
  }, [isOpen, API_BASE_URL]);

  useEffect(() => {
    if (!isOpen) return;
    const prefix = 'JM', last = jobs
      .map(j => j.work_order).filter(id => id?.startsWith(prefix))
      .map(id => parseInt(id.replace(prefix, ''))).filter(n => !isNaN(n))
  ...
  const handleSubmit = async e => {
    ...
    const payload = {
      ...formData,
      contractor: u?.contractor || '', role: u?.role || '', status: 'Pending',
      machines: [formData.machines], timezone,
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <CreateJobForm
          formData={formData} users={users} machines={machines}
