...
export async function loadData(API_BASE_URL) {
  ...
  try {
    const [jobsRes, usersRes] = await Promise.all([
      fetch(`${base}/jobs`), fetch(`${base}/users`)
    ]);
...
export async function updateJobStatus(id, overrideStatus = null, API_BASE_URL) {
  ...
  try {
    ...
    const job = await jobRes.json(), now = new Date().toISOString();
    let status = job.status, contractorStatus = job.contractor_status;

    if (overrideStatus) {
      if (['Approved', 'Completed'].includes(overrideStatus)) status = contractorStatus = 'Completed';
      else if (['Pending', 'Rejected'].includes(overrideStatus)) status = contractorStatus = 'Pending';
      else throw new Error('Invalid override status');
    } else {
      if (status === 'Pending') status = contractorStatus = 'In Progress';
      else if (status === 'In Progress') { status = 'Completed - Pending Approval'; contractorStatus = 'Completed'; }
...
export async function deleteJob(id, API_BASE_URL) {
  const res = await fetch(`${getBaseUrl(API_BASE_URL)}/jobs/${id}`, { method: 'DELETE' });
...
export async function moveJobToInProgress(jobId, API_BASE_URL) {
  ...
  try {
    const res = await fetch(`${getBaseUrl(API_BASE_URL)}/jobs/${jobId}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
      body: JSON.stringify({
        status: 'In Progress', contractor_status: 'In Progress',
        status_timestamp: now, onsite_time: now
      })
    });
