...
class Job(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            **{c.name: getattr(self, c.name) for c in self.__table__.columns if not isinstance(getattr(self, c.name), datetime)},
            "required_date": fmt(self.required_date),
            "completion_date": fmt(self.completion_date),
            "status_timestamp": fmt(self.status_timestamp),
            "onsite_time": fmt(self.onsite_time),
